...
| ADR-016 | 板卡健康监测与可插拔恢复机制 | Scenario 生命周期级 Detection/Decision/Execution 三层分离：BoardHealthMonitor 只监测（HEALTHY/SUSPECTED/UNRESPONSIVE，不调 PowerSwitch）；RecoveryCoordinator 只协调（恢复策略/次数限制/Context 失效/环境收敛）；RecoveryBackend 适配恢复能力（首实现 PowerCycleBackend 复用 ADR-015 PowerSwitch）；配置三分离（system 硬件能力/modules 参数/scenarios 策略）；全部能力默认关闭、向后兼容（已实施·恢复链真机验证通过） |
