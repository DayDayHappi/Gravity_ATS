...
分支 `new_arch`，Scenario 层重构完成（4 个提交）+ 20260824 改动（normal 移除重复 ftp task、修复 no-interactive-wifi 断链），**均尚未真机验证**。

## Current Architecture

场景驱动的分层执行模型：config（三层）→ ScenarioManager（编排）→ Runner（调度）→ Module（动作）→ Driver（通信）。
WiFi 属 prepare 环境准备（wifi_connect 收敛器 + wifi_check 状态检测器），不作 task（ADR-008）。
...
- **已决策待实施**：WiFi 职责重划（ADR-008 Accepted）——normal tasks 移除 wifi_check/scan/join、wifi_connect 改收敛器（见 next_step.md P0-1）。
