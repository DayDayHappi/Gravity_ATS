...
ADR-016 引入应用服务层（Application Service）：BoardHealthMonitor / RecoveryCoordinator / RecoveryBackend，跨模块系统级协调，已实施（devlog `20260921_1820`），运行时闭环已修复（devlog `20260922_1330`），第二轮验收新增问题待修复（2026-09-22）。
...
- 板卡健康监测与可插拔恢复机制（ADR-016）已实施（devlog `20260921_1820`）：`ATS/application/`（board_health_monitor + recovery_coordinator + recovery_backends + runtime_control）+ `core/` 接线（context/logger/scenario/scenario_manager/runner/reporter/main）+ `config/modules/board_health.yaml`；**运行时闭环已修复**（devlog `20260922_1330`，7 P0 + 6 P1 全过），**第二轮验收新增问题待修复**（[第二轮验收](../03_development/archive/ADR016_第二轮验收新增问题与修改建议.md)：2 P0 + 2 P1 + 1 P2 + HW-RISK + QA-GAP）

## Working On

- **板卡健康监测与可插拔恢复机制（ADR-016，主体已实施·第二轮验收问题待修复）**：Document Agent 2026-09-21 定稿，Code Agent 实施主体（devlog `20260921_1820`）+ 修复第一轮 7 P0 + 6 P1（devlog `20260922_1330`）。2026-09-22 第二轮验收（[新增问题](../03_development/archive/ADR016_第二轮验收新增问题与修改建议.md)）新增 2 P0（恢复失败无 framework FAIL 致退出码可能 0、policy 未完整 fail-closed）+ 2 P1（生命周期接线未前置校验、mock 未固化）+ 1 P2 + HW-RISK-01（PowerCycle 后串口是否重枚举）+ QA-GAP-01（无 recovery_validation 场景），待 Code Agent 按 Stage 1~7 修复后再真机。
