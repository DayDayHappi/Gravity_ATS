...
| 日期 | 文件 | 说明 |
|------|------|------|
| 2026-09-23 | [20260923_1318_recovery_validation照搬stress_tasks_loop_preview.md](20260923_1318_recovery_validation照搬stress_tasks_loop_preview.md) | recovery_validation 照搬 stress tasks/loop/preview/rtmp duration：新增 loop enable count:200、preview enabled:true、tasks 整段替换为 stress 24 个 task（photo×10+video×12+video_integrity×1+rtmp duration600，逐项比对一致）；prepare 保持冷启动顺序仅插 preview_start、cleanup 插 preview_stop；health_monitor/recovery 核心与 prepare 冷启动顺序不变；契约校验通过，纯配置改动留痕，待真机 |
| 2026-09-23 | [20260923_1110_BUG006_serial_reconnect探测与旧句柄冲突修复.md](20260923_1110_BUG006_serial_reconnect探测与旧句柄冲突修复.md) | BUG-006 修复：serial_reconnect 探测前先 console.close() 释放旧句柄（避免探测 open 与旧 reader 抢同一端口致 multiple access on port→30s 超时误判未重新枚举）；原端口仍在可访问列表时直接 reconnect 原端口不探测（省 30s 空等），仅真重枚举才走 bounded wait 探测；架构红线不变，新增离线单测覆盖「旧句柄持有原端口」场景，37 测试全过待真机复现 |
| 2026-09-22 | [20260922_1603_ADR016电源控制冷启动与串口生命周期修复.md](20260922_1603_ADR016电源控制冷启动与串口生命周期修复.md) | ADR-016 冷启动/串口生命周期修复（NEW-P0-05/06）：system.yaml 增 serial.power_on_detect_* + power_switch.port；power_switch_init 改 fail-closed（recovery 必需时未启用/无候选/探测失败/ON 未确认均 ScenarioError）+ 上电确认；serial_init 增冷启动 bounded polling 等 EVB UART 枚举；SerialConsole.reconnect 保持对象身份+listener+seq 单调；新增 serial_reconnect action；_restore_environment 移除隐式 board_ready prepend 严格按 YAML 顺序；power_cycle 移除非严格 fallback；contract 升级 power_switch_init<serial_init<health_monitor_start + system.power_switch.enabled 校验；runner continue 加 success 校验；recovery_validation 新顺序；36 测试全过待真机冷启动/运行恢复验收 |
| 2026-09-22 | [20260922_1423_ADR016第三轮验收修复.md](20260922_1423_ADR016第三轮验收修复.md) | ADR-016 第三轮验收修复：NEW-P0-03 runner 增加 continue 分支（恢复成功不重跑直接下个 task，消除「校验允许 continue 但 runner 当未知 action」不一致）；NEW-P0-04 power_switch.py 新增 reboot_checked（下电验合法帧+上电严验 POWER_STATE_ON，失败抛 PowerSwitchError），power_cycle backend 改调它；NEW-P1-03 抽 validate_scenario 公开入口供 run/dry-run 共用；QA-GAP-02 根因修复 .gitignore 移除 /tests/ 忽略规则（tests/ 30 unittest 纳入 git）；tests 增 7 用例至 30 全过，待 HW-RISK-01 真机 |
| 2026-09-22 | [20260922_1404_ADR016第二轮验收修复.md](20260922_1404_ADR016第二轮验收修复.md) | ADR-016 第二轮验收修复：NEW-P0-01 恢复失败生成 board_recovery ERROR 保证退出码非0（Runner 落位，Coordinator 不感知报告）；NEW-P0-02 validate 补 max_attempts>=1/after_recovery/on_exhausted 枚举校验 + Runner 未知 action fail-closed；NEW-P1-01 _validate_scenario_runtime_contract 前置校验（enabled 必须真实接线生命周期 action，不自动补）；NEW-P1-02 固化 tests/（unittest 23 用例：monitor/coordinator/runner/reporter）；NEW-P2-01 watchdog 改 stop_event.wait 立即唤醒+锁外 logger+stop 清 recovery；QA-GAP-01 新增 recovery_validation.yaml 验收场景；HW-RISK-01 留真机确认串口重枚举；架构红线不变，23 测试全过待 TC-BH/TC-RCV 真机 |
