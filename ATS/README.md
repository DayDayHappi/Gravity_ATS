...
- **`Record Start` 偶发漏打，`f_index` 才是编码真实心跳**：实测连续压测后期 `dfs_video_start` 后无 `Record Start` 但 `f_index` 持续增长（编码在跑）。`f_index` 每 90 帧发一次（第一次 `=0` 第二次 `=90`），录像启动判据应以 `f_index` 为准，不能只看 `Record Start`（见 known_issue）
