...
class RtmpModule(TestModule):
    ...
    def run(self, ctx, console, params=None):
        ...
        # 关键字符串检测（推流窗口）：命中不判 FAIL，仅在结果 detail 标注（cycle/rep 由 runner 填）。
        self._hit_monitor = build_monitor(self.config.get("detect_strings"))
        if self._hit_monitor is not None:
        ...
        # 关键字符串 listener 摘除（推流窗口结束）；teardown 仍兜底一次。
        if self._hit_monitor_cb is not None:
            console.remove_listener(self._hit_monitor_cb)
        ...
        res.detail = mon.attach_to(res.detail) if mon else res.detail
        return res
